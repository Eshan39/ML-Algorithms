...
public class ItemsetGenerator<I> {
	
		 public ItemsetData<I> generate(List<Set<I>> transactionList, double minimumSupport) {
			...
			return new ItemsetData<>(extractFrequentItemsets(map), supportCountMap, minimumSupport, transactionList.size());
	}
	...
	private List<Set<I>> generateCandidates(List<Set<I>> itemsetList) {
		...
		for (int i = 0; i < listSize; ++i) {
			...
			Set<I> candidate = concateItemsets(list.get(i), list.get(j));
	...
	private Set<I> concateItemsets(List<I> itemset1, List<I> itemset2) {
		int length = itemset1.size();
		
		for (int i = 0; i < length - 1; ++i) {
			if (!itemset1.get(i).equals(itemset2.get(i))) {
				//System.out.println(itemset1.get(i)+" " +itemset2.get(i));
				//System.out.println(itemset2.get(i));
				return null;
			}
		}
		
		if (itemset1.get(length - 1).equals(itemset2.get(length - 1))) {
			//System.out.println(itemset1.get(length-1)+" " +itemset2.get(length-1));
			return null;
		}
		...
		for (int i = 0; i < length - 1; ++i) {
			//System.out.println(itemset1.get(i));
			ret.add(itemset1.get(i));
		}
//		System.out.println(itemset1.get(length - 1));
//		System.out.println(itemset2.get(length - 1));
